...
class ImagesToBase64Video:
    ...
    def _normalize_frames(self, images):
        if isinstance(images, list):
            raw_frames = [image_tensor.squeeze(0).detach().cpu().numpy() for image_tensor in images]
        else:
            raw_frames = images.detach().cpu().numpy()
    ...
    def _run_ffmpeg(self, frame_pattern, temp_video_path, fps, output_format, video_quality):
    ...
    def convert_to_base64_video(self, images, fps, output_format, video_quality):
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                temp_video_path = os.path.join(temp_dir, f"output.{output_format}")

                frames = self._normalize_frames(images)
                if len(frames) == 0:
                    return ("",)

                for index, frame in enumerate(frames):
                    frame_path = os.path.join(temp_dir, f"frame_{index:06d}.png")
                    Image.fromarray(frame, "RGB").save(frame_path)

                frame_pattern = os.path.join(temp_dir, "frame_%06d.png")
                self._run_ffmpeg(frame_pattern, temp_video_path, fps, output_format, video_quality)
...
class ImagesToBase64List:
    ...
    RETURN_TYPES = ("STRING",)
    RETURN_NAMES = ("base64_list",)
    FUNCTION = "convert_images_to_base64_list"
    CATEGORY = "Image/Convert"

    def convert_images_to_base64_list(self, images):
        """画像を改行区切りのbase64文字列に変換

        - 入力がリスト(images=list)の場合: 各要素(1枚画像テンソル)を順にエンコード
        - 入力がテンソル(images=tensor)の場合: バッチ次元(B)を列挙して各フレームをエンコード
        - 画像はPNGとしてエンコード（data URIプレフィックスは付与しない）
        """
        try:
            base64_strings = []
            ...
            print(f"ImagesToBase64List Error: {e}")
            return ("",)
...
NODE_CLASS_MAPPINGS = {
    ...
    "ImagesToBase64List": ImagesToBase64List,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "ImagesToBase64List": "Images to Base64 List",
